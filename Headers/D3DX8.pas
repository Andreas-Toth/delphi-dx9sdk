...
{*                                                                            *}
{*  Direct3DX 8.1 TMT pascal adaptation by Alexey Barkovoy                    *}
{$ELSE}
{*  Content:    Direct3DX 8.0b headers                                        *}
{*                                                                            *}
{*  Direct3DX 8.0b TMT pascal adaptation by Alexey Barkovoy                   *}
{$ENDIF}
{$ELSE}
{$IFDEF DX81}
{*  Content:    Direct3DX 8.1 headers                                         *}
...
uses
  Windows, ActiveX,
  {$IFDEF TMT}Strings, D3D8{$ELSE}SysUtils, Direct3D8{$ENDIF}, DXFile;
...
{$ELSE}
  d3dx8dll ={$IFDEF DEBUG} 'd3dx8d.dll'{$ELSE} 'D3DX8ab.dll'{$ENDIF};
{$ENDIF}
...
//-------------------------------------------------------------------------
{$IFDEF TMT}
// D3DXGetErrorStringCharA - is "internal" function and shouldn't be used directly
function D3DXGetErrorStringCharA(hr: HResult; pBuffer: PAnsiChar; BufferLen: LongWord): HResult; stdcall; external d3dx8dll name 'D3DXGetErrorStringA';
function D3DXGetErrorStringW(hr: HResult; pBuffer: PWideChar; BufferLen: LongWord): HResult; stdcall; external d3dx8dll name 'D3DXGetErrorStringW';
overload D3DXGetErrorStringA = D3DXGetErrorStringCharA;
overload D3DXGetErrorString = D3DXGetErrorStringCharA;

// Object Pascal support functions for D3DXGetErrorString
function D3DXGetErrorStringPascalA(hr: HResult): String;
// TMT compiler doesn't support WideString -> no "D3DXGetErrorStringPascalW"
overload D3DXGetErrorStringA = D3DXGetErrorStringPascalA;
...
// Object Pascal support functions for D3DXGetErrorString
{$IFDEF FPC}
function {$IFDEF TMT}D3DXGetErrorStringPascalA{$ELSE}D3DXGetErrorStringA{$ENDIF}(hr: HResult): String; overload;
{$ELSE}
function {$IFDEF TMT}D3DXGetErrorStringPascalA{$ELSE}D3DXGetErrorStringA{$ENDIF}(hr: HResult): String;
{$ENDIF}
var
