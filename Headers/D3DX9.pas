...
uses
  Windows, ActiveX,
  {$IFDEF TMT}Strings, D3D9{$ELSE}SysUtils, Direct3D9{$ENDIF}, DXTypes{$IFNDEF DX92}, DXFile{$ENDIF};
...
function D3DXCleanMesh({$IFDEF DX92}CleanType: TD3DXCleanType; {$ENDIF}pMeshIn: ID3DXMesh; pAdjacencyIn: PDWord;
...
{$IFDEF TMT}
// TMT Pascal do not support any kind of long strings (>255 chars),
// so XSKINEXP_TEMPLATES and XEXTENSIONS_TEMPLATES currently are not supported
{$ELSE}
const
  XSKINEXP_TEMPLATES =
        'xof 0303txt 0032' +
...
{$ENDIF}
  ...
  ID3DXAllocateHierarchy = {$IFDEF TMT}object{$ELSE}class{$ENDIF}{$IFDEF FPC}(ID3DXAllocateHierarchy_FPC){$ENDIF}
    ...
    function CreateMeshContainer(Name: PAnsiChar; const pMeshData: TD3DXMeshData;
        pMaterials: PD3DXMaterial; pEffectInstances: PD3DXEffectInstance;
        NumMaterials: DWORD; pAdjacency: PDWORD; pSkinInfo: ID3DXSkinInfo;
        out ppNewMeshContainer: PD3DXMeshContainer): HResult; {$IFNDEF TMT}virtual; stdcall; abstract;{$ELSE}stdcall; virtual;{$ENDIF}
...
{$ENDIF}
  ...
  ID3DXSaveUserData = {$IFDEF TMT}object{$ELSE}class{$ENDIF}{$IFDEF FPC}(ID3DXSaveUserData_FPC){$ENDIF}
    ...
    function AddMeshChildData(pMeshContainer: PD3DXMeshContainer;
        pXofSave: {$IFDEF DX92}ID3DXFileSaveObject{$ELSE}IDirectXFileSaveObject{$ENDIF}; pXofMeshData: {$IFDEF DX92}ID3DXFileData{$ELSE}IDirectXFileData{$ENDIF}): HResult; {$IFNDEF TMT}virtual; stdcall; abstract;{$ELSE}stdcall; virtual;{$ENDIF}
    ...
    function AddTopLevelDataObjectsPre(pXofSave: {$IFDEF DX92}ID3DXFileSaveObject{$ELSE}IDirectXFileSaveObject{$ENDIF}): HResult; {$IFNDEF TMT}virtual; stdcall; abstract;{$ELSE}stdcall; virtual;{$ENDIF}
    function AddTopLevelDataObjectsPost(pXofSave: {$IFDEF DX92}ID3DXFileSaveObject{$ELSE}IDirectXFileSaveObject{$ENDIF}): HResult; {$IFNDEF TMT}virtual; stdcall; abstract;{$ELSE}stdcall; virtual;{$ENDIF}

    // callbacks for the user to register and then save templates to the XFile
    function RegisterTemplates(pXFileApi: {$IFDEF DX92}ID3DXFile{$ELSE}IDirectXFile{$ENDIF}): HResult; {$IFNDEF TMT}virtual; stdcall; abstract;{$ELSE}stdcall; virtual;{$ENDIF}
    function SaveTemplates(pXofSave: {$IFDEF DX92}ID3DXFileSaveObject{$ELSE}IDirectXFileSaveObject{$ENDIF}): HResult; {$IFNDEF TMT}virtual; stdcall; abstract;{$ELSE}stdcall; virtual;{$ENDIF}
  end;
...
{$ENDIF}
  ...
  ID3DXAnimationCallbackHandler = {$IFDEF TMT}object{$ELSE}class{$ENDIF}{$IFDEF FPC}(ID3DXAnimationCallbackHandler_FPC){$ENDIF}
    ...
    //
    //----------------------------------------------------------------------------
    function HandleCallback(Track: LongWord; pCallbackData: Pointer): HResult; {$IFNDEF TMT}virtual; stdcall; abstract;{$ELSE}stdcall; virtual;{$ENDIF}
  end;
...
{$IFDEF TMT}
/// TMT Pascal do not support "abstract" methods in classes, so... ///
...
function ID3DXSaveUserData.AddFrameChildData(pFrame: PD3DXFrame;
  pXofSave: {$IFDEF DX92}ID3DXFileSaveObject{$ELSE}IDirectXFileSaveObject{$ENDIF};
