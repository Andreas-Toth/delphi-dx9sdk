...
{$ENDIF}
uses
  Windows, MMSystem, ActiveX, DXTypes, {$IFDEF TMT}DSound{$ELSE}DirectSound{$ENDIF};
...
//Translator: IReferenceClock is already defined in DirectSound.pas
  IReferenceClock = {$IFDEF TMT}DSound{$ELSE}DirectSound{$ENDIF}.IReferenceClock;
...
{$IFDEF BYTE2_ALIGNMENT}{$IFNDEF TMT}{$MINENUMSIZE 1}{$ELSE}{$Z1}{$ENDIF}{$ENDIF}
